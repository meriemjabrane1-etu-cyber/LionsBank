                ...
                <div className="mx-auto max-w-[1400px] space-y-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3 bg-white dark:bg-white/5 p-2 rounded-2xl border border-slate-200 dark:border-white/10 backdrop-blur-sm shadow-sm dark:shadow-none">
                            ...
                            <div>
                                <p className="text-xs text-slate-400 dark:text-white/40 uppercase font-bold tracking-wider">Active Alerts</p>
                                <p className="text-sm font-bold text-orange-600 dark:text-orange-500">{stats.empty} ATMs Empty</p>
                            </div>
                        </div>
                    </div>

                    {/* Stats Cards */}
                    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:grid-cols-5">
                        {[
                            { label: 'Total Fleet', value: stats.total, color: 'current', icon: HardDrive },
                            { label: 'Online', value: stats.active, color: 'rgb(28,212,132)', icon: CheckCircle, glow: true },
                            { label: 'Out of Cash', value: stats.empty, color: 'rgb(244, 63, 94)', icon: AlertTriangle },
                            { label: 'In Maintenance', value: stats.maintenance, color: 'rgb(8, 145, 178)', icon: Settings },
                            { label: 'Offline', value: stats.out_of_service, color: 'rgb(251, 146, 60)', icon: WifiOff },
                        ].map((stat, i) => (
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {filteredAtms.map((atm) => (
                            <Card key={atm.id} className="bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 hover:border-[rgb(28,212,132)]/30 transition-all group overflow-hidden shadow-sm dark:shadow-none">
                                <CardContent className="p-6 space-y-6">
                                    {/* ATM Header */}
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-3">
                                            <div className="h-12 w-12 rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-center justify-center text-[rgb(28,212,132)] group-hover:bg-[rgb(28,212,132)] group-hover:text-[#041F1E] transition-all">
                                                <HardDrive className="h-6 w-6" />
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-lg text-slate-900 dark:text-white">{atm.name}</h3>
                                                <div className="flex items-center gap-1 text-slate-400 dark:text-white/40 text-xs font-medium">
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider">
                                            <span className="text-slate-400 dark:text-white/40">Cash Availability</span>
                                            <span className={atm.cash_available < 20 ? 'text-rose-600 dark:text-rose-500' : 'text-emerald-600 dark:text-[rgb(28,212,132)]'}>
                                                {atm.cash_available}%
                                            </span>
                                        </div>
                                    ...
                                    <div className="flex gap-3 pt-2">
                                        <Button 
                                            ...
                                            className="flex-1 bg-slate-100 dark:bg-white/10 hover:bg-[rgb(28,212,132)] hover:text-[#041F1E] border border-slate-200 dark:border-white/5 transition-all gap-2 rounded-xl h-10 font-bold uppercase tracking-wider text-xs text-slate-600 dark:text-white"
                                        >
                                        ...
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                                <Button variant="ghost" className="h-10 w-10 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10">
                                                    <Settings className="h-4 w-4 text-slate-400 dark:text-white/60" />
                                                </Button>
                                            </DropdownMenuTrigger>
                                            <DropdownMenuContent className="bg-white dark:bg-[#062B29] border-slate-200 dark:border-white/10 text-slate-900 dark:text-white shadow-xl">
                                                <DropdownMenuItem onClick={() => updateStatus(atm.id, 'active')} className="focus:bg-slate-100 dark:focus:bg-white/10 font-bold text-xs uppercase tracking-widest">
                                                    <CheckCircle className="mr-2 h-4 w-4 text-[rgb(28,212,132)]" />
                                                    Set Active
                                                </DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => updateStatus(atm.id, 'maintenance')} className="focus:bg-slate-100 dark:focus:bg-white/10 font-bold text-xs uppercase tracking-widest">
                                                    <Settings className="mr-2 h-4 w-4 text-cyan-600 dark:text-cyan-500" />
                                                    Maintenance Mode
                                                </DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => updateStatus(atm.id, 'empty')} className="focus:bg-slate-100 dark:focus:bg-white/10 text-rose-600 dark:text-rose-500 font-bold text-xs uppercase tracking-widest">
                                                    <AlertTriangle className="mr-2 h-4 w-4" />
                                                    Mark as Empty
                                                </DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => updateStatus(atm.id, 'out_of_service')} className="focus:bg-slate-100 dark:focus:bg-white/10 text-orange-600 dark:text-orange-400 font-bold text-xs uppercase tracking-widest">
                                    ...
                                    <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-white/5 text-[10px] uppercase font-bold tracking-widest text-slate-400 dark:text-white/20">
                                        <div className="flex items-center gap-1.5">
                                            {atm.status === 'active' ? <Wifi className="h-3 w-3 text-emerald-600 dark:text-[rgb(28,212,132)]" /> : <WifiOff className="h-3 w-3 text-rose-500" />}
                        ...
                        {filteredAtms.length === 0 && (
                            <div className="col-span-full flex flex-col items-center justify-center p-20 bg-white/40 dark:bg-white/5 rounded-3xl border border-dashed border-slate-200 dark:border-white/10 text-slate-400 dark:text-white/30">
                                ...
                                <p className="font-bold uppercase tracking-widest text-sm text-slate-600 dark:text-white/50">No ATMs Found</p>
